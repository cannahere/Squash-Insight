...
export default function MatchDetail({ match, onUpdate, onReport }) {
  ...
  function startAnalyze(){
    ...
    intervalRef.current = setInterval(()=>{
      ...
      try {
        ...
        const alpha = 0.05; // slow, stable baseline
        ...
        // split when quiet for 600ms and ≥1.6s since last split (more responsive)
        if (idleMs > 600 && (v.currentTime - lastSplit) > 1.6){
          const start = lastSplit, end = v.currentTime;
          lastSplit = end;
          pushRally(start,end);
  ...
  function incr(group,key){ onUpdate(m=>({ ...m, tallies:{ ...m.tallies, [group]:{ ...m.tallies[group], [key]:(m.tallies[group][key]||0)+1 } } })); }
  function decr(group,key){ onUpdate(m=>({ ...m, tallies:{ ...m.tallies, [group]:{ ...m.tallies[group], [key]:Math.max(0,(m.tallies[group][key]||0)-1) } } })); }
    ...
    <div className='grid two'>
      <div className='card'>
        ...
        <div className='row' style={{marginTop:8}}>
          <button className='btn' onClick={()=>{const v=videoRef.current;if(!v)return;v.paused?v.play():v.pause();}}>Play/Pause <span className='small'>(<kbd>Space</kbd>)</span></button>
          <button className='btn primary' onClick={manualSplit}>Manual Split <span className='small'>(<kbd>Enter</kbd>)</span></button>
          {!analyzing ? (
...
Add start/stop analyze + adaptive auto rally detection
